...
class Cascatador(MetaData):
    def __init__(self, data):
        ...
        for c in self.casos:
            ...
            qafl_usinas_mean = qafl_usinas.loc[qafl_usinas["cenario"] == "mean"].round(1)

            qtur_usinas = self.eco_indicadores.retorna_df_concatenado("QTUR_UHE_EST")
            qtur_usinas_mean = qtur_usinas.loc[qtur_usinas["cenario"] == "mean"].round(1)

            qver_usinas = self.eco_indicadores.retorna_df_concatenado("QVER_UHE_EST")
            qver_usinas_mean = qver_usinas.loc[qver_usinas["cenario"] == "mean"].round(1)
    ...
    def add_scatter_graph(self,lista_traces ,no, nivel, d_hidr, defluencia_usinas_mean_est, qincr_usinas_mean_est, qafl_usinas_mean_est, qtur_usinas_mean_est, qver_usinas_mean_est):
        ...
        for pai in pais:
            ...
            texto = pai.nome + "<br> QINC:"+str(qinc) + " QAFL:"+str(qafl) + "<br>QT/V/D: "+ str(qtur) + " "+ str(qver) + " " + str(deflu) 

            simbolo = self.retorna_simbolo(pai, d_hidr)
            lista_traces.append(go.Scatter(x = [pai.x], y = [pai.y], text=[ texto ], textfont=dict( size=9), textposition= pai.text_position, mode = "markers+text", marker_color="rgba(0,0,255,1.0)" , marker=dict(symbol=simbolo, size=20)))
