...
class Cascatador(MetaData):
    ...
    def add_scatter_graph(self,fig ,no, nivel):
        ...
        for pai in pais:
            ...
            #if(nivel < 5):
            fig.add_trace(go.Scatter(x = [pai.x], y = [pai.y], text=[pai.nome], textfont=dict( size=15), textposition="top right", mode = "markers+text", marker_color="rgba(0,0,255,1.0)" , marker=dict(symbol="triangle-down", size=20)))
