...
class InfoGeralOperDecomp(Estruturas):
    ...
    def preenche_operacao(self,caso):
        ...
        if(os.path.isfile(caso.caminho+"/relato."+extensao)):
            ...
            h_tempo =    0 if ultimo_tempo < 3600 else math.floor(ultimo_tempo/(60*60))
            min_tempo =    round(ultimo_tempo/60,0)  if h_tempo == 0 else  round(ultimo_tempo/(60) - h_tempo*60 ,0)
            temp = temp.replace("Tempo", str(h_tempo)+ " h " + str(min_tempo) + " min")
            ...
            #temp = temp.replace("CMO_SE (avg)",     str(cmo_SE_mean                        )  )
            #temp = temp.replace("CMO_NE (avg)",     str(cmo_NE_mean                     )  )
            temp = temp.replace("CMO_SE (1 est)",   str(round(cmo_SE_1_est       ,2)         )  )
            temp = temp.replace("CMO_NE (1 est)",   str(round(cmo_NE_1_est       ,2)         )  )
