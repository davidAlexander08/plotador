...
class GraficosCalibracaoCVAR:
    ...
    def __init__(self, casos: List[CasoCalibracaoCVAR]):
        self.casos = casos
    ...
    def __gera_pareto(
        ...
        for c in self.casos:
            fig.add_trace(
                go.Scatter(
                    x=[df.at[c.nome, eixo_x]],
                    y=[df.at[c.nome, eixo_y]],
                    mode="markers+text",
                    text=c.nome,
                    marker_color=c.cor,
                    marker_symbol=c.marcador,
    ...
    def gera_pareto_cdef_cter(
        self, pmo: str, df_tabela_valores_medios: pd.DataFrame
    ) -> go.Figure:
        """
        Gera o gráfico de fronteira de pareto entre Custo de Défitict
        e Custo de Geração Térmica. Recebe o DataFrame com
        os valores médios do horizonte do estudo, onde o nome
        de cada caso deve estar no index. Colunas esperadas:

        - cdef_med_r
        - cterm_med_r$

        """
        Log.log().info("Gerando pareto de CDEF x CTER")
        return self.__gera_pareto(
            df_tabela_valores_medios,
            "cterm_med_r$",
            "cdef_med_r$",
            "Custo GT sem VPL (R$)",
            "Custo DEF sem VPL (R$)",
            f"Fronteira de Pareto: Custo GT x Custo DEF - {pmo}",
        )

    def gera_pareto_earm_cter(
        self, pmo: str, df_tabela_valores_medios: pd.DataFrame
    ) -> go.Figure:
        """
        Gera o gráfico de fronteira de pareto entre EARM e Custo
        de Geração Térmica. Recebe o DataFrame com os valores médios
        do horizonte do estudo, onde o nome de cada caso deve estar
        no index. Colunas esperadas:

        - earm_med_mwmes
        - cterm_med_r$

        """
        Log.log().info("Gerando pareto de CTER x EARM")
        return self.__gera_pareto(
            df_tabela_valores_medios,
            "cterm_med_r$",
            "earm_med_mwmes",
            "Custo GT sem VPL (R$)",
            "EARM Médio (MWmes)",
            f"Fronteira de Pareto: Custo GT x EARM Médio - {pmo}",
        )

    def gera_pareto_ct_delta_earm(
        self, pmo: str, df_valores_incrementais: pd.DataFrame
    ) -> go.Figure:
        """
        Gera o gráfico de fronteira de pareto entre a variação de Custo de
        Geração Térmica e a variação de Energia Armazenada com relação
        ao caso referência. Recebe o DataFrame com
        as variações médias no horizonte de estudo, onde o nome
        de cada caso deve estar no index. Colunas esperadas:
        ...
        """
        Log.log().info("Gerando pareto de CT x Delta EARM")
        return self.__gera_pareto(
            df_valores_incrementais,
            "delta_earm_mwmes",
            "cterm_med_r$",
            "Ganho EARM (MWmes)",
            "Custo GT sem VPL (R$)",
            f"Fronteira de Pareto: Ganho EARM x Custo GT - {pmo}",
        )
    ...
    def gera_pareto_ct_delta_earm(
        ...
        """
        Log.log().info("Gerando pareto de Delta CT x Delta EARM")
        return self.__gera_pareto(
            df_valores_incrementais,
            "delta_earm_mwmes",
            "cterm_med_r$",
    ...
    def gera_pareto_gt_delta_earm(
        self, pmo: str, df_valores_incrementais: pd.DataFrame
    ) -> go.Figure:
        """
        Gera o gráfico de fronteira de pareto entre a variação de
        Geração Térmica e a variação de Energia Armazenada com relação
        ao caso referência. Recebe o DataFrame com
        as variações médias no horizonte de estudo, onde o nome
        de cada caso deve estar no index. Colunas esperadas:
        ...
        """
        Log.log().info("Gerando pareto de Delta GT x Delta EARM")
        return self.__gera_pareto(
            df_valores_incrementais,
            "gt_mwmes",
            "earm_medio_final_perc",
            "EARM SIN Med (%)",
            "GT SIN Med (MWmes)",
            f"Fronteira de Pareto: EARM SIN x GT - {pmo}",
        )
    ...
    def gera_pareto_delta_gt_delta_earm(
        self, pmo: str, df_valores_incrementais: pd.DataFrame
    ) -> go.Figure:
        """
        Gera o gráfico de fronteira de pareto entre a variação de
        ...
        return self.__gera_pareto(
            ...
            "Ganho EARM (MWmes)",
            "Ganho GT (MWmes)",
            f"Fronteira de Pareto: Ganho EARM x Ganho GT - {pmo}",
        )

    def gera_pareto_delta_ct_delta_earm(
        self, pmo: str, df_valores_incrementais: pd.DataFrame
    ) -> go.Figure:
        """
        Gera o gráfico de fronteira de pareto entre a variação de Custo de
    ...
    def gera_grafico_barra_primeiro_ano(
        self,
        df: pd.DataFrame,
        unidade: UnidadeSintese
        ) ->go.Figure:
    
        dados = df.loc[df["tipo"]==unidade.nomeSintese]
        anoInicial = dados.index.tolist()[0][1]
        dados = dados.reset_index(drop=True).drop(columns=["tipo"])
        ...
        fig = self.gera_grafico_barra(
            ...
            unidade.legendaEixo,
            1,
            unidade.titulo+"_"+str(anoInicial)
        )
        return fig
            


    def gera_grafico_barra_medios_outros_anos(
        self,
        df: pd.DataFrame,
        unidade: UnidadeSintese
        ) ->go.Figure:
        ...
        dados = df.loc[df["tipo"]==unidade.nomeSintese]
        anoOutrosInicial = dados.index.tolist()[1][1]
        anoOutrosFinal = dados.index.tolist()[-1][1]
        dados = dados.reset_index(drop=True).drop(columns=["tipo"])
        
        fig = self.gera_grafico_barra(
            dados.iloc[1:].mean().values , 
            dados.columns.values.tolist(), 
            "casos", 
            unidade.legendaEixo,
            1,
            unidade.titulo+"_"+str(anoOutrosInicial)+"-"+str(anoOutrosFinal)
        )
        return fig
